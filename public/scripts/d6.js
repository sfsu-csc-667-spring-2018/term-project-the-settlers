...
D6Animator.getImageBank = function(key, baseUrl) {
	...
	if (!imageBank) {
		...
		imageBank.blank.src = baseUrl + "/images/dice/blank.gif";
		var i;
		for (i=1; i<7; ++i) {
			whichDie = "die" + i;
			imageBank[whichDie] = new Object();
			imageBank[whichDie].die = new Image();
			imageBank[whichDie].die.src = baseUrl + "/images/dice/die-" + i + ".gif";
			imageBank[whichDie].side = new Image();
			imageBank[whichDie].side.src = baseUrl + "/images/dice/dices-" + i + ".gif";
			imageBank[whichDie].top = new Image();
			imageBank[whichDie].top.src = baseUrl + "/images/dice/dicet-" + i + ".gif";
		}
	}
	return imageBank;
}
...
D6AnimBuilder.prototype.genDiceHtml = function(layout, callback, callbackData) {
	...
	for (var i=0; i<layout.length; ++i) {
		...
		for (var j=0; j<imgsThisRow; ++j) {
			...
			if (dieCount > numTotalImgs) break;
			if (this.useImages) {
				genHtml += "<img id='" + this.id + dieCount + "' class='die' src='" + this.baseUrl + "/images/dice/blank.gif' />";
			} else {
